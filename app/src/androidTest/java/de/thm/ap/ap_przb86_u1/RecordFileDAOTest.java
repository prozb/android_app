...
public class RecordFileDAOTest {
   ...
   public void setUp() throws Exception {
      ...
      recordFileDAO = new RecordFileDAO(ctx);
   ...
   public void findAll() throws Exception {
      List<Record> records = recordFileDAO.findAll();
      assertNotNull(records);
      assertEquals(4, recordFileDAO.findAll().size());
   }
   ...
   public void findById() throws Exception {
      ...
      Optional<Record> optRecord = recordFileDAO.findById(records.get(0).getId());
   ...
   public void update() throws Exception {
      ...
      record.setIsHalfWeighted(false);
      record.setCredits(3);
      record.setMark(51);

      assertTrue(recordFileDAO.update(record));

      Optional<Record> optRecordUpdate = recordFileDAO.findById(record.getId());
      ...
      assertEquals(record.getCredits(), recordUpdate.getCredits());
      ...
      assertFalse(recordFileDAO.update(recordNoId));
   }

   @Test
   public void persist() throws Exception {
      Record record = new Record("CS1022", "Betriebssysteme", 2017, false, false, 6, 90);
      Integer id = recordFileDAO.persist(record);

      Optional<Record> optRecord = recordFileDAO.findById(id);
      assertTrue(optRecord.isPresent());
      ...
      assertEquals(record.getCredits(), recordNew.getCredits());
